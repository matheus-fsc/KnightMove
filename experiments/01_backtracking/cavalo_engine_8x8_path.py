#!/usr/bin/env python3
"""
cavalo_engine_8x8_path.py  v2
CORRECAO FUNDAMENTAL:  GF(2) cycle space so produz graus pares. Caminho S->E tem grau impar em S e E.
  SOLUCAO: adicionar aresta virtual (S,E) -> converte path em cycle no grafo aumentado.
  Assinatura = 168 dims (ativacao de cada aresta original).
"""
import json, os, math
...
def solve_path(nodes, edges_orig, start, end, n_amostras):
...
def main():
    ...
    for start,end in pares:
        ...
        sigs,es=solve_path(nodes,edges,start,end,AMOSTRAS_POR_PAR)
    ...
    with open("resultados_8x8.json","w") as f: json.dump(out,f,indent=2)
    print("\nSalvo: resultados_8x8.json")
