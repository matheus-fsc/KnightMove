...
paralelizada via multiprocessing.Pool — versão parametrizada por board.

Algoritmo (por caminho, O(V × max_deg × α(V))):
  - reconstrói o caminho hamiltoniano da assinatura (walk O(V))
  - simula a remoção em ORDEM REVERSA via union-find (path-halving + rank)
  - rastreia (V, E, C) ao longo dos n+1 estados; H₁ = E - V + C
  - Δ H₁ no passo forward t = H₁_history[n-t] - H₁_history[n-t-1]

Uso:
  ../.venv/bin/python destruction_full.py --board 10 --data-dir data_parallel
  ../.venv/bin/python destruction_full.py --board 10 --expand-d4 --workers 16
"""
...
# ── módulo-global, populado por _init_board (antes do Pool) ───────────
...
def _build_canonical_edges(adj, nodes):
    s = set()
    for v in nodes:
        for u in adj[v]:
...
def _d4_edge(e, t, board):
    u, v = e
    u2, v2 = _d4_node(u, t, board), _d4_node(v, t, board)
    return (u2, v2) if u2 <= v2 else (v2, u2)


def _init_board(board):
    """Popula as globais. Chamado uma vez antes do Pool.fork()."""
    global BOARD, NODES, ADJ, DEGREE, EDGES, N_EDGES, D4_REMAPS
    BOARD = board
    ADJ = _build_adj(board)
    NODES = list(ADJ.keys())
    DEGREE = {v: len(ADJ[v]) for v in NODES}
    EDGES = _build_canonical_edges(ADJ, NODES)
    N_EDGES = len(EDGES)
    edge_to_idx = {e: i for i, e in enumerate(EDGES)}
    D4_REMAPS = [
        [edge_to_idx[_d4_edge(e, t, board)] for e in EDGES]
        for t in range(8)
    ]
    ...
    for sig in b["signatures"]:
        ...
        if expand_d4:
            for t in range(1, 8):
                ...
                s_t = _d4_node(s, t, BOARD)
                e_t = _d4_node(e, t, BOARD)
...
def heatmap_grid(values_dict, fmt, board):
    ...
    for r in range(board):
        ...
        print(f"  rank {rank_lbl:>2}  " + "  ".join(row))
...
def main():
    ...
    print("=" * 70)
    print(f"DESTRUIÇÃO DE LOOPS — {args.board}×{args.board} (TODAS AS AMOSTRAS)")
    print("=" * 70)
    print(f"  data_dir     : {args.data_dir}")
    print(f"  lotes        : {len(paths)}")
    print(f"  workers      : {args.workers}")
    print(f"  expand_d4    : {args.expand_d4}")
    print(f"  V={len(NODES)}, E={N_EDGES}, H1={N_EDGES - len(NODES) + 1}")
    print()
    if not paths:
    ...
    with ctx.Pool(processes=args.workers,
                  initializer=_init_board,
                  initargs=(args.board,)) as pool:
        for result in pool.imap_unordered(process_batch_file, tasks,
            ...
            if progress % 200 == 0 or progress == len(tasks):
                ...
                print(f"  [{progress}/{len(tasks)}] "
                      f"paths={global_stats['n_processed']:>11,}  "
    ...
    print(f"── HEATMAP — DESTRUIÇÃO MÉDIA POR CASA ──")
    print()
    heatmap_grid(means, "{:6.3f}", args.board)
    print()

    print(f"── HEATMAP — DESTRUIÇÃO MÁXIMA POR CASA ──")
    print()
    heatmap_grid(maxes, "{:>5d}  ", args.board)
    print()
    ...
    print()
    print(f"  {'passo':>5}  {'N':>11}  {'%':>6}  {'cum%':>6}  histograma")
    cum = 0
    for k in sorted(peak):
        c = peak[k]
        cum += c
        ...
        print(f"  {int(k):>5d}  {c:>11,}  {pct:>5.1f}%  {cum_pct:>5.1f}%  {bar}")
    print()
